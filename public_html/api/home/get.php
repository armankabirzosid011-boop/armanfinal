<?php
/**
 * Home Content Get API
 * 
 * GET /api/home/get.php
 * 
 * Retrieves home page content from the database (site_settings table).
 * This endpoint is public for reading home content.
 * 
 * Response:
...
try {
    ...
    // Fetch all home-related setting keys
    $stmt = $db->query("SELECT setting_key, setting_value, setting_group, updated_at FROM site_settings 
                        WHERE setting_key IN (
                            ...
                            'aboutSpecialties', 'aboutAffiliations',
    ...
    foreach ($settings as $s) {
        ...
        } elseif (str_starts_with($key, 'emergency')) {
            if (!isset($data['emergencySection'])) $data['emergencySection'] = [];
            $data['emergencySection'][$key] = $value;
        }
    }
    
    // Ensure default structure even if some keys are missing
    $data['heroSection'] = $data['heroSection'] ?? [];
    $data['aboutSection'] = $data['aboutSection'] ?? [];
    $data['footerSection'] = $data['footerSection'] ?? [];
    $data['emergencySection'] = $data['emergencySection'] ?? [];
    ...
    error_log('Home get error: ' . $e->getMessage());
    errorResponse('Failed to fetch home content', 500);
}
