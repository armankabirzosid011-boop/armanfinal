...
export function loadFromStorage<T>(key: string): T[] {
  try {
    const result = await storage.getItem(key); raw = result === null ? null : result;
    if (!raw) return [];
...
export function getVisitFormData(visitId: string | number | null, email?: string): Record<string, any> | null {
  ...
  try {
    const raw = await storage.getItem(`visit_form_data_${id}_${usedEmail}`);
...
export function generateRegisterNumber(): string {
  const counter = Number.parseInt(await storage.getItem('medicare_register_counter') || '') + 1;
  await storage.setItem('medicare_register_counter', String(counter));
...
export function setPrescriptionHeaderImage(type: string, imageDataUrl: string, doctorEmail?: string): void {
  const email = doctorEmail ?? getDoctorEmail();
  const key = `prescriptionHeaders_${type}_${email}`;
  await storage.setItem(key, imageDataUrl);
}
...
export function loadPrescriptionRecords(patientId: number | string): any[] {
  try {
    const raw = await storage.getItem(prescriptionRecordsKey(patientId));
    if (!raw) return [];
...
export function savePrescriptionRecords(patientId: number | string, records: any[]): void {
  try {
    await storage.setItem(prescriptionRecordsKey(patientId), JSON.stringify(records));
  } catch {
    // silently ignore
  }
}
...
function loadDrugReminders(patientId: number | string): any[] {
  try {
    const raw = await storage.getItem(drugRemindersKey(patientId));
    if (!raw) return [];
    return JSON.parse(raw);
  } catch {
    return [];
...
function saveDrugReminders(patientId: number | string, reminders: any[]): void {
  try {
    await storage.setItem(drugRemindersKey(patientId), JSON.stringify(reminders));
  } catch {
    // silently ignore
  }
}
